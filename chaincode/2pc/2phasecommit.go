...
func (s *SmartContract) Invoke(APIstub shim.ChaincodeStubInterface) sc.Response {
	...
	} else if function == "verdictTx" {
		return s.verdictTx(APIstub, args)
	}
...
func (s *SmartContract) initiateTx(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	if len(args) < 2 {
		return shim.Error("Incorrect number of arguments. Expecting 3 or more")
	}
	
	//marshall test
	var tx Tx 
	tx = Tx{State: "Voting", VoteN: 0, StartTime: time.Now()}
	tx.Addr = make([]string, len(args)-1)
...
func (s *SmartContract) voteTx(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	if !stringInSlice(args[1],tx.Addr) {
		return shim.Error("Adress not allowed to Voted")
	}
...
func (s *SmartContract) verdictTx(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
